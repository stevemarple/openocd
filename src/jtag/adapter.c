...
settings for srst and trst in sync with values set by the "adapter reset_config"
command.
*/
...
COMMAND_HELPER(helper_adapter_gpio_print_config, enum adapter_gpio_config_index gpio_idx)
{
	...
	command_print(CMD, "adapter gpio %s (%s): num %d, chip %d, active-%s%s%s",
...
COMMAND_HANDLER(adapter_gpio_config_handler)
{
	...
	if (!adapter_config.gpios_initialized) {
		for (int j = 0; j < ADAPTER_GPIO_IDX_NUM; ++j) {
			adapter_config.gpios[j].gpio_num = -1;
			adapter_config.gpios[j].chip_num = -1;
			if (gpio_map[j].direction == ADAPTER_GPIO_DIRECTION_INPUT)
				adapter_config.gpios[j].init_state = ADAPTER_GPIO_INIT_STATE_INPUT;
		}
		...
		/* JTAG GPIOs should be inactive expect for tms */
